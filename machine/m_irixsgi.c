...

{
	...
	{
		/*
		 * Note: this is _compiled_ on 6.x where x >= 4 but I would like it to
		 * run on 6.2 6.3 as well (backward binary compatibility). Scheduling
		 * is completely different between these IRIX versions and some
		 * scheduling classes may even have different names.
		 *
		 * The solution: have more than one style of 'priority' depending on
		 * the OS version.
		 *
		 * See npri(1) + nice(2) + realtime(5) for scheduling classes, and
		 * priority values.
		 */
...
char	   *ordernames[] = {
	/*
	 * Aliases for user convenience/friendliness: mem == size rss == res
	 */
...
compare_cpu(struct prpsinfo ** pp1, struct prpsinfo ** pp2)
{
	...
	/*
	 * order by various keys, resorting to the next one whenever there's a tie
	 * in comparisons
	 */
	ORDERKEY_PCTCPU
		ORDERKEY_CPTICKS
