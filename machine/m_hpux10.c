...

{
	...
	/*
	 * Allocate space for the per-process structures (pst_status).	To make
	 * life easier, simply allocate enough storage to hold all the process
	 * information at once.  This won't normally be a problem since machines
	 * with lots of processes configured will also have lots of memory.
	 */
	nproc = info.max_proc;
...

{
	...
	/*
	 * CPU times to avoid space problems, we roll SWAIT (kernel semaphore
	 * block) into BLOCK (spin lock block) and SSYS (kernel process) into SYS
	...
	/*
	 * If we can get mpid from the kernel, then we will do so now. Otherwise
	 * we'll guess at mpid from the most recently started process time.  Note
	 * that this requires us to get the pst array now rather than in
	 * get_process_info().	We rely on get_system_info() being called before
	 * get_system_info() for this to work reliably.
	 */
	for (i = 0; i < nproc; i++)
		pst[i].pst_pid = -1;
...

{
	...
	/*
	 * Eliminate unwanted processes and tot up all the wanted processes by
	 * state
	 */
	...
	for (total = 0, active = 0, i = 0; pst[i].pst_pid >= 0; i++)
	{
		...
		/*
		 * If we are eliminating idle processes, then a process is regarded as
		 * idle if it is in a short term sleep and not using much CPU, or
		 * stopped, or simple dead.
		 */
		if (!sel->idle
